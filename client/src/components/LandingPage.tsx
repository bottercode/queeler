...
const LandingPage = () => {
  ...
  const onHandleGoogleSignin = () => {
    ...
    window.open('https://main--enchanting-basbousa-f2b256.netlify.app/auth/google', '_self')
  }
  return (
    <div className='flex justify-center items-center h-screen bg-[#E9EAEC]'>
      <div className='border drop-shadow-2xl shadow-2xl bg-[#e9eaec9d] h-[60%] w-[50%] flex flex-col items-center'>
        <div className='flex items-center mt-20 gap-48'>
          ...
          <img src={Logo} alt='logo' className='bg-none ml-4 w-[400px] h-[300px]'></img>
        </div>

        {!isLoggedIn && (
          <button className='mt-16 px-4 py-2 bg-[#a3a3a3] text-white rounded-2xl' onClick={onHandleGoogleSignin}>
            Sign In With Google
          </button>
        )}
        {isLoggedIn && (
          <Link className='mt-16 px-4 py-2 bg-[#a3a3a3] text-white rounded-2xl' to='chat'>
            chat
          </Link>
        )}
      </div>
